...
// This personal site intentionally overrides these reviewed theme templates.
const personalOverrides = new Set(["_includes/news.liquid", "_layouts/about.liquid", "_layouts/bib.liquid"]);
const onlyPersonalOverrides = (dir) =>
  fs.statSync(path.join(root, dir)).isDirectory() && fs.readdirSync(path.join(root, dir)).every((name) => personalOverrides.has(`${dir}/${name}`));
...
for (const forbiddenPath of ["_includes", "_layouts", "_sass", "_scripts", "assets/tailwind", "tailwind.config.js", "assets/webfonts"]) {
  if (exists(forbiddenPath) && !onlyPersonalOverrides(forbiddenPath)) {
